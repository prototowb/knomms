...
> Status: **shipped in v0.7.0** (2026-08-06, KC-065–070). §9 verification executed in full
> (37 checks, `scripts/verify-v070.sh`), all green. Covers four of the five Tier 4 candidates from
...
| OQ-18 | What `editor` unlocks | KB: add sources (URL + upload). Asset: commit new versions. Harness: add/swap slots, submit evals (and read the runs they can now trigger). Everything else — visibility, metadata PATCH, deprecate, grant management — stays owner-only. Forking follows readability, as it always has (a viewer may fork what they can read) | Narrow, enumerated write surface via a `get_editable_by_id` twin of `get_readable_by_id` per domain. Grant management itself is owner-only so an editor cannot escalate |
