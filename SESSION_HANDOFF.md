...
**State:** v0.13.0 released — multi-source synthesis part 1; everything KC-032–098 verified (cloud enabled-path still pending an operator API key)  
**Branch:** PR #15 (v0.11.0+v0.12.0) merged 2026-08-07; v0.13.0 rides the next `development` → `main` PR; tags v0.10.0–v0.13.0 pushed  
**Tests:** 213/213 backend (pytest) · 0 TypeScript errors (vue-tsc)  
**Live verification:** through KC-070 via the 37-check three-user script (`scripts/verify-v070.sh`); v0.8.0 disabled-path + v0.9.0 study-KB + v0.10.0 cohort + v0.11.0 gates + v0.12.0 video + v0.13.0 synthesis (`scripts/verify-v0130.py`) green on Colima. Migration head: **018** (v0.12.0/v0.13.0 are schema-free).  
**Stack:** Running on Colima (macOS) — see §Dev Runtime

**v0.13.0 (2026-08-07):** multi-source synthesis part 1 (`docs/16-multi-source-synthesis.md`, OQ-63–68). `POST /v1/kbs/{id}/synthesize` = balanced per-source retrieval (`retrieve()` grew a `source_id` filter; `SYNTHESIS_CHUNKS_PER_SOURCE` default 2) + ONE comparison generation — iterative hop loops deliberately deferred (CPU generation ≈ 2 min each). SSE events are byte-identical to `/query` (OQ-67) — `useStreamingQuery(kbId, 'synthesize')` is the same composable, and `SynthesisService` reuses `GenerationService._generate_stream` (same semaphore). Guards: readable-KB 404; membership/count/duplicate 422 via pure `check_source_selection` — source ids are validated against the KB *and* retrieval stays namespace-scoped, so a foreign id can't pull another KB's chunks even if the guard slipped. Gotcha fixed live: the KB Ask/Compare renderer only recognised `[SOURCE:uuid]` — the local model sometimes emits bare `[uuid]`, which rendered as raw text (learn page had handled both since KC-018); both renderers now match both forms, and any new citation-rendering surface must too.

**v0.12.0 (2026-08-06):** video transcript ingestion part 1 (`docs/15-video-ingestion.md`, OQ-53–62). YouTube URLs become `type="video"` sources at submit time (the worker dispatches extractors on the type — it must be right at creation); the worker branch calls `VideoExtractor.fetch_and_extract` and never GETs the watch page. Transcript preference: manual > auto-generated, English first (OQ-57); blocks are ~400-char accumulations with `ts:HH:MM:SS` locators (the RawBlock contract had reserved these since M1). Caption-less/unavailable videos raise ValueError → `failed` status (ASR is part 2). `ChunkSearchResult.source_url` + `SourceOut.raw_url` feed the `watch?t=Ns` deep links (`frontend/utils/video.ts` — NOT auto-imported; import explicitly, this project's vue-tsc doesn't resolve utils/ auto-imports). **Found+fixed live: the original ingestion submit paths enqueued the Redis job before committing the Source row** — a fast worker logged "Source not found — skipping job" and the source sat `pending` forever; both `submit_url`/`submit_file` now commit-before-enqueue (KC-077's rule, applied to where it originated). Colima note: `docker image prune -f` before rebuild rounds — dangling `--no-cache` layers filled the VM disk once this session.
