...
**Generated by Proto Gear v0.10.0**
**Project**: 
**Ticket Prefix**: KC
...
**Local-Only Workflow** - No remote repository configured
...
#### Feature Branches: `feature/KC-{number}-{description}`

**Examples**:
```
feature/KC-001-implement-auth
feature/KC-002-add-dashboard
```
...
#### Bugfix Branches: `bugfix/KC-{number}-{description}`

**Examples**:
```
bugfix/KC-010-fix-login-error
bugfix/KC-011-resolve-crash
```
...
git checkout -b feature/KC-XXX-description development
```
...
git checkout -b feature/KC-042-add-search development
...
git merge feature/KC-042-add-search
git branch -d feature/KC-042-add-search
```
...
git checkout -b feature/KC-XXX development
...
- Reference issues: `Closes KC-XXX`
...
3. **Create feature branch**: `git checkout -b feature/KC-XXX-description`
...
- Reference ticket numbers (KC-XXX)
...
```markdown
I'm going to:
1. Create branch: feature/KC-XXX-feature-name
...
### Examples
```
KC-001: Implement user authentication
KC-002: Add dashboard widgets
KC-003: Fix login redirect issue
```
...
```
BRANCH PATTERNS:
feature/KC-XXX-description    → New features
bugfix/KC-XXX-description     → Bug fixes
